...
```{r }
my_name <- c("M","u","h","a","m","m","a","d", "H","a","r","i","s")
my_name
```
...
```{r }
even <- matrix(1:9 * 2, nrow = 3, ncol = 3)
even
```


```{r }
row.names(even) <- c("A","B","C")
colnames(even) <- c("X","Y","Z")
even
```
...
```{r }
bank_customers <- data.frame(CustomerID = c("Alicia","Nancy","Fernando","Louis","Diane"), HasAccount = c("No","Yes","Yes","Yes","No"), TotalBalance =c("NA",10000,"NA",2000,"NA"))
bank_customers
```
